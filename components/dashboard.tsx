...
export function Dashboard() {
  const [isVisualize, setIsVisualize] = useState(true);
  ...
  return (
    <div className="flex min-h-screen" style={{ background: 'linear-gradient(150deg, #f0f4f8 0%, #e6ecf3 100%)' }}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <button
          onClick={() => setIsVisualize(!isVisualize)}
          className="fixed bottom-8 right-8 z-50 px-5 py-3 text-sm font-bold text-white rounded-2xl shadow-xl transition-all hover:scale-105 hover:shadow-2xl flex items-center gap-2"
          style={{ background: isVisualize ? 'linear-gradient(135deg,#2563eb,#1d4ed8)' : 'linear-gradient(135deg,#7c3aed,#6d28d9)' }}
        >
          {isVisualize ? (
            <>
